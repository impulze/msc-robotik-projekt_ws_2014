...
def main():
	...
	while True:
		...
		if line[:-1] == 'quit':
			...
			_m5 = m5(dh_parameters['theta5'], dh_parameters['d5'], dh_parameters['a5'])
			_m6 = m6(dh_parameters['theta6'], dh_parameters['d6'], dh_parameters['a6'])
			_mt = mt(0, 0)
			point = numpy.matrix([[dh_parameters['px']], [dh_parameters['py']], [dh_parameters['pz']], [1]])
			m06 = _m1.dot(_m2).dot(_m3).dot(_m4).dot(_m5).dot(_m6)
			m03 = _m1.dot(_m2).dot(_m3)
			print(numpy.linalg.inv(m03))
			mres = m06
			beta = calc_beta(mres)
			alpha = calc_alpha(mres, beta)
			gamma = calc_gamma(mres, beta)
			print 'Alpha: %f' % alpha	
			print 'Beta: %f' % beta
			print 'Gamma: %f' % gamma
			m0t = mres.dot(_mt)
